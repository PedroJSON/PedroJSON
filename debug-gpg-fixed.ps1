...
if ($lines.Length -lt 5) {
    Write-Host "Key appears to be on too few lines (possibly corrupted line endings)" -ForegroundColor Yellow
}
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        if ($signingPassword) {
            $signResult = "test content" | & gpg --batch --yes --passphrase $signingPassword --pinentry-mode loopback --armor --detach-sign 2>&1
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-Host "Sign result: $signResult" -ForegroundColor Yellow
            }
        } else {
            Write-Host "SIGNING_PASSWORD not set, skipping signing test" -ForegroundColor Yellow
        }
    } else {
        Write-Host "GPG import failed" -ForegroundColor Red
    }
...
$keyId = "B4B2C6F2B6154719E1CC7C3CDA642791E9611F13"
Write-Host "gpg --armor --export-secret-keys $keyId" -ForegroundColor Cyan